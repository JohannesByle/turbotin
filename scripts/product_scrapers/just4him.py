        ...
        while next_page:
            ...
            for product in products:
                if "There are no products to list in this category." not in str(product):
                    ...
                    # print(product.find(align = "right").get_text())
                    if product.find(align="right").get_text().strip() != "Add:":
                        price = \
                        re.findall(r"\$\d+\.\d{2}", product.find_all(class_="productListing-data")[-1].get_text())[0]
                    stock = "In stock"
                    link = product.find("a").get("href")
                    item, price, stock, link = add_item(data, name, item, price, stock, link, pbar)
            next_page = False
            if new_soup.find("div", class_="navSplitPagesLinks forward"):
                for pages in new_soup.find("div", class_="navSplitPagesLinks forward"):
                    for page in pages:
                        if page == "[Next >>]":
                            next_page = True
                            new_soup = get_html(pages.get("href"))
